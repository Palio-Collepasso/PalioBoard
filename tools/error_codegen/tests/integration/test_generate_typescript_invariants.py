...
formatting, API runtime behavior, or backend generation details that belong to
the Python domain-error tests.
"""
...
def test_typescript_generation_exports_catalog_metadata_contract(
    ...
    for entry in entries:
        entry_block = extract_balanced_block(rendered, f"  {entry.code}: {{")
        assert_tokens_in_order(
            entry_block,
            [
                f'code: "{entry.code}"',
...
def test_typescript_generation_exposes_context_keys_for_interpolation(
    ...
    for entry in entries:
        ...
        context_block = extract_balanced_block(rendered, f"export type {alias_name} =")
        ...
        assert_tokens_in_order(context_block, [path[0] for path in top_level_paths])
