...
- `src/palio/api/` for FastAPI routers and request middleware, with `src/palio/bootstrap/` as the explicit composition root and app/runtime assembly
- `src/palio/settings.py` for the typed env-based runtime settings used by the app/bootstrap and DB helpers
- `src/palio/bootstrap/db/` for runtime/migration DB configuration and runtime assembly, plus `src/palio/infrastructure/db/` for concrete reusable DB implementations such as the SQLAlchemy Unit of Work
