...
.PHONY: help up down api-dev web-dev openapi-export openapi-types format format-check lint typecheck check-boundaries check-openapi build verify test test-api test-web test-e2e
...
api-dev: ## Run the api locally with uv
	$(call require_file,$(API_DIR)/pyproject.toml,the api app,TASK-2)
...
format: ## Format api code
	$(call require_file,$(API_DIR)/pyproject.toml,the api tooling,TASK-10)
...
test-api: ## Run api tests
	$(call require_file,$(API_DIR)/pyproject.toml,the api test harness,TASK-8)
